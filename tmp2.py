...
def main(args):
    ...
    gg = args.truc * (w_styles - latent_avg) + latent_avg
    gg = gg.unsqueeze(1).repeat(1, 18, 1)

    output_latents = gg.clone().detach()
    ...
    style_ckpt = torch.load(os.path.join('pretrained_models', 'ffhq.pt'), map_location=args.device)
    generator.load_state_dict(style_ckpt["g_ema"], strict=True)
    with torch.no_grad():
        outputs = generator([gg], input_is_latent=True)[0]
        nrows = int(args.n_sample ** 0.5)
        nrows = args.n_sample
        save_image(
            outputs,
